...
class WaybackDownloader:
    ...
    def _get_file_type(self, url):
        ...
        if any(url_lower.endswith(ext) for ext in ['.jpg', '.jpeg', '.png', '.gif', '.bmp', '.svg', '.webp', '.ico']):
            return 'image'
        elif url_lower.endswith('.css'):
            return 'css'
        elif url_lower.endswith('.js'):
            return 'js'
        elif any(url_lower.endswith(ext) for ext in ['.html', '.htm', '.php', '.asp', '.aspx']) or '.' not in Path(urlparse(url).path).name:
            return 'html'
        else:
            return 'other'
    ...
    def _url_to_filepath(self, url):
        ...
        if parsed.query:
            path = path.replace('?', '_') + '_' + parsed.query.replace('&', '_').replace('=', '_')
    ...
    def _rewrite_urls_in_html(self, html_content, base_url):
            ...
            for tag in soup.find_all(tag_name):
                if tag.has_attr(attr):
                    ...
                    if 'web.archive.org/web/' in original_url:
                        ...
                        match = re.search(r'web\.archive\.org/web/\d+/(.*)', original_url)
                        if match:
                    ...
                    if self._is_internal_url(absolute_url):
                        ...
                        try:
                            relative_path = os.path.relpath(local_path, current_path.parent)
        ...
        for script in soup.find_all('script'):
            if script.string and ('archive.org' in script.string or 'wombat' in script.string.lower()):
                script.decompose()
    ...
    def _crawl_page(self, original_url):
        ...
        if file_type == 'html':
            rewritten_html, resources = self._rewrite_urls_in_html(response.text, original_url)
            ...
            for link in soup.find_all('a', href=True):
                ...
                if self._is_internal_url(absolute_url) and self._get_file_type(absolute_url) == 'html':
                    if absolute_url not in self.visited_urls:
                        self.url_queue.append(absolute_url)

            # Add resources to queue
            for resource_url in resources:
                if resource_url not in self.visited_urls and self._get_file_type(resource_url) != 'html':
    ...
    def download(self):
        ...
        print(f"\n{'='*60}")
        print(f"Wayback Machine Downloader")
        print(f"{'='*60}")
